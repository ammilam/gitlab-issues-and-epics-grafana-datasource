...
import { findMostCommonElement } from './processing';
...
export async function fetchIssuesAndEpicsFromGitlab(apiUrl: string, groupId: number, accessToken: string) {
  ...
  try {
    ...
    const response: any = await fetch(apiUrl + `/api/grapql`, {
      headers: {
    ...
    let gitlabIssues = group.issues.nodes;
    let gitlabEpics = group.epics.nodes;
